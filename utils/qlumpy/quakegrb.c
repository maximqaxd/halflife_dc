...
Grabs a rectangle out of the loaded image and stores it as a Dreamcast texture
image rather than a mip chain of palette indices.  The hardware only samples
powers of two, so anything else is scaled on the way in.
==============
*/
void GrabPVR (void)
{
	byte	*source, *screen_p;
	byte	*pvr;
	int		xl, yl, w, h;
	int		width, height;
	int		x, y, size;
	...
	width = PVR_NearestSize (w);
	height = PVR_NearestSize (h);
	if (width > pvrmaxsize)
		width = pvrmaxsize;
	if (height > pvrmaxsize)
		height = pvrmaxsize;

	// only square textures can be quantized, so a lopsided one is worth
	// squaring up whenever the quantized copy comes out smaller
	size = width > height ? width : height;
	if (PVR_EncodedSize (size, size, PVR_WITH_MIPMAPS) < PVR_EncodedSize (width, height, PVR_WITH_MIPMAPS))
	{
		width = size;
		height = size;
	}

	// point sample into the size the hardware wants; palette indices can't be
	// blended, so the closest source texel wins
	source = malloc (width * height);
	for (y = 0; y < height; y++)
	{
		screen_p = byteimage + (yl + (y * h) / height) * byteimagewidth + xl;
		for (x = 0; x < width; x++)
			source[y * width + x] = screen_p[(x * w) / width];
	}

	pvr = PVR_EncodeIndexed (source, lbmpalette, width, height, PVR_WITH_MIPMAPS, &size);

	if (size > MAXLUMP)
		Error ("line %i: %s came to %d bytes, too big for a lump", scriptline, lumpname, size);

	memcpy (lump_p, pvr, size);
	lump_p += size;

	if (width != w || height != h)
		printf ("%-16s %dx%d scaled to %dx%d, %d bytes\n", lumpname, w, h, width, height, size);
	else
		printf ("%-16s %dx%d, %d bytes\n", lumpname, width, height, size);
