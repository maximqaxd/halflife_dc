...
void WriteBoneInfo( )
{
	...
	if (neomodel)
	{
		// a Neo model only carries the parts an attachment is looked up by
		mstudioattachment_neo_t *pneoattachment = (mstudioattachment_neo_t *)pData;

		for (i = 0; i < numattachments; i++) {
			pneoattachment[i].bone		= attachment[i].bone;
			VectorCopy( attachment[i].org, pneoattachment[i].org );
		}
		pData += numattachments * sizeof( mstudioattachment_neo_t );
	}
	else
	{
				...
				for (j = 0; j < numbones; j++)
				{
					for (k = 0; k < 6; k++)
					{
						...
						else if (neomodel)
						{
							...
							panimvalue = (mstudioanimvalue_t *)NeoPackAnimation( (byte *)panimvalue,
								sequence[i].panim[q]->anim[j][k],
								sequence[i].panim[q]->numanim[j][k], k >= 3 );
						}
						else
						{
							panim->offset[k] = ((byte *)panimvalue - (byte *)panim);
...
void WriteTextures( )
{
	...
	for (i = 0; i < numtextures; i++) {
		// every texture starts on a long boundary so the loader can read the
		// image header straight out of the cached model
		ALIGN( pData );
...
void WriteFile (void)
{
	...
	if (split_textures)
	{
		...
		phdr->id = neomodel ? IDSTUDIONEOHEADER : IDSTUDIOHEADER;
