...
#include "studiomdl.h"
#include "..\..\src\halflife\activity.h"
#include "..\..\src\halflife\activitymap.h"
...
void TextureCoordRanges( s_mesh_t *pmesh, s_texture_t *ptexture  )
{
	...
	if (ptexture->flags & STUDIO_NF_CHROME) {
		...
		for (i=0 ; i<pmesh->numtris ; i++) {
			...
			ptexture->min_t = 0;
			if (ptexture->ispvr) {
				// the chrome map is whatever size the artist made it
				ptexture->max_s = ptexture->skinwidth - 1;
				ptexture->max_t = ptexture->skinheight - 1;
			}
			else {
				ptexture->max_s = 63;
				ptexture->max_t = 63;
	...
	// find the range
	if (!clip_texcoords && !ptexture->ispvr)
	{
		for (i=0 ; i<pmesh->numtris ; i++) {
			for (j = 0; j < 3; j++) {
...
Textures can only be uploaded at power of two sizes, so pick the one the skin
is closest to.  256 is as large as a skin is allowed to get.
===============
*/
int NearestPowerOfTwo( int value )
{
	...
	for (size = 8; size < 256; size = size * 2)
	{
...
Point samples an 8 bit skin to a new size.  The palette indices can't be
blended, so the closest source texel wins.  Rows come in padded out to a
multiple of four, the way the BMP loader leaves them.
===============
*/
byte *ResampleSkin( byte *ppicture, int srcwidth, int srcheight, int width, int height )
{
	byte	*pdest;
	byte	*pout;
	int		srcadjwidth;
	...
	srcadjwidth = (srcwidth + 3) & ~3;
	pdest = malloc( width * height );
	pout = pdest;

	for (i = 0; i < height; i++)
	{
		t = (i * srcheight) / height;
		for (j = 0; j < width; j++)
		{
			s = (j * srcwidth) / width;
			*(pout++) = *(ppicture + s + t * srcadjwidth);
		}
	}
...
void Grab_BMP ( char *filename, s_texture_t *ptexture )
{
	...
	if (pvrtextures)
	{
		// the hardware only takes powers of two, so the skin is resampled on
		// the way in and the texture coordinates follow it
		byte	*presampled;
		int		width, height;

		width = NearestPowerOfTwo( ptexture->srcwidth );
		height = NearestPowerOfTwo( ptexture->srcheight );
		presampled = ResampleSkin( ptexture->ppicture, ptexture->srcwidth, ptexture->srcheight,
			width, height );

		if (width != ptexture->srcwidth || height != ptexture->srcheight)
			printf ("resampled %s from [%d %d] to [%d %d]\n", ptexture->name,
				ptexture->srcwidth, ptexture->srcheight, width, height );

		ptexture->srcwidth = width;
		ptexture->srcheight = height;
		ptexture->pdata = PVR_EncodeIndexed( presampled, (byte *)ptexture->ppal,
			width, height, PVR_NO_MIPMAPS, &ptexture->size );
		ptexture->ispvr = 1;

		free( presampled );
		free( ptexture->ppicture );
		free( ptexture->ppal );
		ptexture->ppicture = NULL;
		ptexture->ppal = NULL;
	}
}


void Grab_PVR ( char *filename, s_texture_t *ptexture )
{
	ptexture->pdata = PVR_LoadFile( filename, &ptexture->size,
...
void ResizeTexture( s_texture_t *ptexture )
{
	...
	if (ptexture->ispvr)
	{
		// the texture is already in the format the hardware wants, so there is
		// nothing to crop or repack; it goes into the model as it came in
		ptexture->skintop = 0;
		ptexture->skinleft = 0;
		ptexture->skinwidth = ptexture->srcwidth;
		ptexture->skinheight = ptexture->srcheight;

		printf ("PVR %s [%d %d]  %6d bytes\n", ptexture->name, ptexture->skinwidth,
			ptexture->skinheight, ptexture->size );
		return;
	}
...
void Grab_Skin ( s_texture_t *ptexture )
{
	...
	else if (stricmp( ".pvr", &file1[strlen(file1)-4]) == 0) {
		Grab_PVR( file1, ptexture );
	}
	else {
...
void ParseScript (void)
{
	while (1)
	{
		...
		else if (!strcmp (token, "$pvrtextures"))
		{
			pvrtextures = 1;
		}
		else if (!strcmp (token, "$neomodel"))
		{
			neomodel = 1;
		}
...
int main (int argc, char **argv)
{
	...
	if (argc == 1)
		Error ("usage: studiomdl [-t texture] -r(tag reversed) -n(tag bad normals) -f(flip all triangles) [-a normal_blend_angle] -h(dump hboxes) -i(ignore warnings) -p(convert skins to PVR) -e(neo model) [-g max_sequencegroup_size(K)] file.qc");
