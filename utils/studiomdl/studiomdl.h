...
#define IDSTUDIOSEQHEADER	(('Q'<<24)+('S'<<16)+('D'<<8)+'I')
														// little-endian "IDSQ"
#define IDSTUDIONEOHEADER	(('t'<<24)+('s'<<16)+('d'<<8)+'i')
														// little-endian "idst"
...
EXTERN	int			pvrtextures;
EXTERN	int			neomodel;

// A Neo model stores its animation packed instead of as loose words, and drops
// the parts of an attachment nothing reads back.
typedef struct
{
	int			bone;
	vec3_t		org;
...
typedef struct 
{
	...
	int		ispvr;			// already a Dreamcast texture image, copy it through untouched
